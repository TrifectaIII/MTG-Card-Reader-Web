...
class GmsMatcher:
    ...
    def knnMatch(self, img1, img2, k=2):
    ...
    def get_nb9(self, idx, grid_size):
        ...
        for yi in range(-1, 2):
            for xi in range(-1, 2):
                ...
                if (
                    idx_xx < 0
                    or idx_xx >= grid_size.width
                    or idx_yy < 0
                    or idx_yy >= grid_size.height
                ):
                    continue
    ...
    def set_scale(self, scale):
        self.grid_size_right.width = (
            self.grid_size_left.width * self.scale_ratios[scale]
        )
        self.grid_size_right.height = (
            self.grid_size_left.height * self.scale_ratios[scale]
        )
        self.grid_number_right = (
            self.grid_size_right.width * self.grid_size_right.height
        )
    ...
    def run(self, rotation_type):
        ...
        for GridType in range(1, 5):
            ...
            for i in range(self.matches_number):
                if (
                    self.cell_pairs[int(self.match_pairs[i][0])]
                    == self.match_pairs[i][1]
                ):
        ...
        if drawing_type == DrawingType.ONLY_LINES:
            for i in range(len(self.gms_matches)):
                ...
                right = tuple(
                    sum(x)
                    for x in zip(
                        self.keypoints_image2[self.gms_matches[i].trainIdx].pt,
                        (src1.shape[1], 0),
                    )
                )
                cv2.line(
                    output, tuple(map(int, left)), tuple(map(int, right)), (0, 255, 255)
                )
            ...
            for i in range(len(self.gms_matches)):
                ...
                cv2.line(
                    output, tuple(map(int, left)), tuple(map(int, right)), (255, 0, 0)
                )
            ...
            drawing_type == DrawingType.COLOR_CODED_POINTS_X
            or drawing_type == DrawingType.COLOR_CODED_POINTS_Y
            or drawing_type == DrawingType.COLOR_CODED_POINTS_XpY
        ):
            ...
            for i in range(len(self.gms_matches)):
                ...
                if drawing_type == DrawingType.COLOR_CODED_POINTS_X:
                    colormap_idx = int(left[0] * 256.0 / src1.shape[1])  # x-gradient
                if drawing_type == DrawingType.COLOR_CODED_POINTS_Y:
                    colormap_idx = int(left[1] * 256.0 / src1.shape[0])  # y-gradient
                if drawing_type == DrawingType.COLOR_CODED_POINTS_XpY:
                    colormap_idx = int(
                        (left[0] - src1.shape[1] * 0.5 + left[1] - src1.shape[0] * 0.5)
                        * 256.0
                        / (src1.shape[0] * 0.5 + src1.shape[1] * 0.5)
                    )  # manhattan gradient
...
if __name__ == "__main__":
    ...
    matches = gms.knnMatch(img1, img2)
